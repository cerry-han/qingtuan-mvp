...
系统拒绝卡片层层嵌套、复杂侧边栏、九宫格入口堆叠和装饰性动效。老人端不设置中转首页，进入后直接打开“和青团说话”。
...
- **Title**（700，1.45rem，1.35）：功能分区与步骤标题。
...
电脑端使用固定左侧导航：品牌与六项老人端任务位于上部，字体、声音和紧急求助固定在底部。右侧只展示当前任务内容，不设置中转首页。窄屏下导航退化为两列入口，保证所有任务仍可直接看到。

### Voice Action

语音是“和青团说话”页面最大、最先出现的操作。按钮同时显示动作“和青团说话”和结果预期“点一下开始，我会认真听”。识别时文字改为明确的监听状态。
...
- **Do** 让六项核心入口与字体、声音、求助保持固定位置和一致文案。
